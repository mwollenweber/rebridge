...
extern "C" DllExport
bool IDAPython_run_string(const char *run_string, 
	...
	//QASSERT(main != NULL);
    globals = GetMainGlobals();
	QASSERT(globals != NULL);
	result = PyRun_String(run_string, Py_file_input, globals, globals);
	//msg("ida_bridge: result is @ 0x%08x: [0x%08x]\n", (int)&result, (int)result);
	if (result == NULL){
		*buf_size = 8096;
		*result_buf = (char *)malloc((size_t)*buf_size);
        handle_python_error(*result_buf, *buf_size);
        return false;
	}
	//str = PyObject_Str(result);
	str = PyObject_Str(globals);
	if(str==NULL){
		*buf_size = 8096;
		*result_buf = (char *)malloc((size_t)*buf_size);
        handle_python_error(*result_buf, *buf_size);
	...
	if (PyString_AsStringAndSize(str, &c_str, (Py_ssize_t *)buf_size) == -1){
		*buf_size = 8096;
		*result_buf = (char *)malloc((size_t)*buf_size);
        handle_python_error(*result_buf, *buf_size);
		Py_XDECREF(result);
	...
	//msg("ida_bridge: Made it to where the buffer is set\n");
	*result_buf = (char *)malloc((size_t)*buf_size);
	memcpy(*result_buf, c_str, *buf_size);
	Py_XDECREF(result);
	Py_XDECREF(str);
	return true;
}
extern "C" DllExport
bool IDAPython_eval_string(const char *run_string, 
						/*in-out*/ char **result_buf, 
						/*in-out*/ unsigned long *buf_size){
    ...
    globals = GetMainGlobals();
	QASSERT(globals != NULL);
	//msg("ida_bridge: Made it to before the satement eval\n");
	result = PyRun_String(run_string, Py_eval_input, globals, globals);
	//msg("ida_bridge: result is @ 0x%08x: [0x%08x]\n", (int)&result, (int)result);
	if (result == NULL){
		*buf_size = 8096;
		*result_buf = (char *)malloc((size_t)*buf_size);
	...
	//msg("ida_bridge: Made it to where the size is set\n");
	if (PyString_AsStringAndSize(str, &c_str, (Py_ssize_t *)buf_size) == -1){
		*buf_size = 8096;
		*result_buf = (char *)malloc((size_t)*buf_size);
        handle_python_error(*result_buf, *buf_size);
	...
	//msg("ida_bridge: Made it to where the buffer is set, buf_size: %d\n",*buf_size);
	*result_buf = (char *)malloc((size_t)*buf_size);
	//msg("ida_bridge: Made it to where the buffer is set, buf is set: 0x%08x\n",(int)result_buf);
	memcpy(*result_buf, c_str, *buf_size);
	Py_XDECREF(result);
	Py_XDECREF(str);
	return true;
...
extern "C" DllExport
bool IDAPython_destroy_object(void *alloced){
	if (alloced != NULL){
		free(alloced);
		return true;
	}
...
extern "C" DllExport
bool IDAPython_compile_eval_string(const char *run_string, 
							const char *filename, 
						/*in-out*/ char **result_buf, 
						/*in-out*/ unsigned long *buf_size){
	...
	QASSERT(globals != NULL);
	//msg("ida_bridge: Made it to before the satement eval\n");
	code = (PyCodeObject *)Py_CompileString(run_string, filename, Py_eval_input);
	if ( code == NULL){
		*buf_size = 8096;
		*result_buf = (char *)malloc((size_t)*buf_size);
        handle_python_error(*result_buf, *buf_size);
...
extern "C" DllExport
bool IDAPython_compile_input_string(const char *run_string, 
							const char *filename, 
						/*in-out*/ char **result_buf, 
						/*in-out*/ unsigned long *buf_size){
	...
	QASSERT(globals != NULL);
	//msg("ida_bridge: Made it to before the satement eval\n");
	code = (PyCodeObject *)Py_CompileString(run_string, filename, Py_file_input);
	if ( code == NULL){
		*buf_size = 8096;
		*result_buf = (char *)malloc((size_t)*buf_size);
        handle_python_error(*result_buf, *buf_size);
