...
void cleanup(bool warn) {
   ...
   if (conn != INVALID_SOCKET) {
      ...
      conn = INVALID_SOCKET;
      if (warn) {
         warning("Connection to the debugger has been closed.\n");
      }
   }
}
...
SOCKET listen_on(const char *host, short port) {
   SOCKET sock;
   sockaddr_in server;
   memset(&server, 0, sizeof(server));
   server.sin_family = AF_INET;
...
bool createSocketWindow(SOCKET s, Dispatcher d) {
   ...
   msg_hwnd = CreateWindowEx(0, TEXT("STATIC"), PLUGIN_NAME, 0, 0, 0, 0, 0, HWND_MESSAGE, 0, 0, 0);
   if (msg_hwnd == NULL) {
...
bool init_network() {
   ...
   msg(PLUGIN_NAME": succeeded to init socket.\n");
#endif
   return true;
}
